...
CREATE TABLE `SIGNATURES` (
  ...
  `TSID` int(11) = (SELECT `UserID` FROM `USER` WHERE `role` = "Exam Setter"),
  `TSSign1` tinyint(4) DEFAULT NULL,
  `IMID` int(11) = (SELECT `UserID` FROM `USER` WHERE `role` = "Internal Moderator"),
  `IMSign` tinyint(4) DEFAULT NULL,# Sequel Pro dump
# Version 1630
# http://code.google.com/p/sequel-pro
#
# Host: 127.0.0.1 (MySQL 5.1.37)
...
CREATE TABLE `SIGNATURES` (
  ...
  `EVCID` int(11) = (SELECT `UserID` FROM `USER` WHERE `role` = "Exam Vetting Committee"),
  `EVCSign` tinyint(4) DEFAULT NULL,
  `TSSign3` tinyint(4) DEFAULT NULL,
  `EEID` int(11) = (SELECT `UserID` FROM `USER` WHERE `role` = "External Examiner"),
...
INSERT INTO `SIGNATURES` (`ExamID`,`TSSign1`,`IMSign`,`TSSign2`,`EVCSign`,`TSSign3`,`EESign`,`TSSignFinal`)
VALUES


 (00000001,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
